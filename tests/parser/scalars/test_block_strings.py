...
def _assert_eq_from_lines(block_lines: List[str], expected_lines: List[str]):
    block_string = '"""{}"""'.format('\n'.join(block_lines))
    expected_string = '\n'.join(expected_lines)
    assert _PARSER.parse(block_string) == ast.StringNode(1, 0, expected_string)
...
def test_empty():
    assert _PARSER.parse('""""""') == ast.StringNode(1, 0, '')
...
def test_escaped_triple_quotes():
    assert _PARSER.parse('"""hello\\"""world"""') == ast.StringNode(1, 0, 'hello"""world')
